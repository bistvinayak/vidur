...
async function callOpenAI(apiKey: string, model: string, messages: unknown[], maxTokens: number, promptText: string): Promise<ProviderResult> {
...
export function createOpenAIProvider(apiKey: string, model: string): ModelProvider {
  return {
    async summarizePage(page: ExtractedPage, opts: ProviderCallOpts) {
      ...
      return callOpenAI(apiKey, model, [{ role: 'user', content }], 700, promptText)
    },
    ...
    async askFollowUp(priorMessages: ChatMessage[], userMessage: string, opts: ProviderCallOpts) {
      ...
      return callOpenAI(apiKey, model, messages, 700, userContent)
    },
